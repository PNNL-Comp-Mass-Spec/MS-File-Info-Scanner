...
// If the following is false, then data is not loaded/saved from/to the DatasetTimeFile.txt or the FolderIntegrityInfo.txt file
	...
	public override event MessageEventEventHandler iMSFileInfoScanner.MessageEvent;
	public override event ErrorEventEventHandler iMSFileInfoScanner.ErrorEvent;
	...
	DateTime static_CheckForAbortProcessingFile_dtLastCheckTime;
	private void CheckForAbortProcessingFile()
	{

		try {
			if (DateTime.UtcNow.Subtract(static_CheckForAbortProcessingFile_dtLastCheckTime).TotalSeconds < 15) {
				return;
			}

			static_CheckForAbortProcessingFile_dtLastCheckTime = DateTime.UtcNow;
		...
		try {
			...
			if (intFileCount > 0) {
				...
				if (mUseCacheFiles && !blnForceRecheck) {
					if (mMSFileInfoDataCache.CachedFolderIntegrityInfoContainsFolder(diFolderInfo.FullName, intFolderID, objRow)) {
						int intCachedFileCount = Convert.ToInt32(objRow(clsMSFileInfoDataCache.COL_NAME_FILE_COUNT));
						int intCachedCountFailIntegrity = Convert.ToInt32(objRow(clsMSFileInfoDataCache.COL_NAME_COUNT_FAIL_INTEGRITY));
				...
				if (blnCheckFolder)
				{
					...
					if (mUseCacheFiles) {
						if (!mMSFileInfoDataCache.UpdateCachedFolderIntegrityInfo(udtFolderStats, intFolderID)) {
							intFolderID = -1;
						}
					}

					WriteFileIntegrityDetails(ref mFileIntegrityDetailsWriter, intFolderID, udtFileStats);
	...
	public List<string> GetKnownFileExtensionsList()
	{
	    ...
	    {
	        clsFinniganRawFileInfoScanner.FINNIGAN_RAW_FILE_EXTENSION,
	        clsAgilentTOFOrQStarWiffFileInfoScanner.AGILENT_TOF_OR_QSTAR_FILE_EXTENSION,
	        clsBrukerXmassFolderInfoScanner.BRUKER_BAF_FILE_EXTENSION,
	        clsBrukerXmassFolderInfoScanner.BRUKER_MCF_FILE_EXTENSION,
	        clsBrukerXmassFolderInfoScanner.BRUKER_SQLITE_INDEX_EXTENSION,
	        clsUIMFInfoScanner.UIMF_FILE_EXTENSION,
	        clsDeconToolsIsosInfoScanner.DECONTOOLS_CSV_FILE_EXTENSION
	    };
	...
	{
		List<string> lstExtensionsToParse = new List<string>();
	...
	{
		if (strBaseMessage == null || strBaseMessage.Length == 0) {
	...
	protected void LogMessage(string strMessage, eMessageTypeConstants eMessageType)
	{
		...
		if (mLogFile == null && mLogMessagesToFile) {
			try {
				if (mLogFilePath == null || mLogFilePath.Length == 0) {
				...
				try {
					...
					if (mLogFolderPath.Length == 0) {
						...
						if ((mOutputFolderPath != null) && mOutputFolderPath.Length > 0) {
	...
	{
		bool blnOpenedExistingFile = false;
		FileStream fsFileStream = null;
		string strDefaultFileName = null;
	...
	public override bool PostDatasetInfoToDB(string strDatasetInfoXML, string strConnectionString, string strStoredProcedure)
	{
		...
		try {
			...
			intStartIndex = strDatasetInfoXML.IndexOf("?>", StringComparison.Ordinal);
			if (intStartIndex > 0) {
			...
			intResult = executeSP.ExecuteSP(objCommand, MAX_RETRY_COUNT, SEC_BETWEEN_RETRIES);
	...
	private bool ProcessMSDataset(string strInputFileOrFolderPath, ref iMSFileInfoProcessor objMSInfoScanner, string strDatasetName, string strOutputFolderPath)
	{
	...
	public bool ProcessMSFileOrFolder(string strInputFileOrFolderPath, string strOutputFolderPath)
	{

		eMSFileProcessingStateConstants eMSFileProcessingState = default(eMSFileProcessingStateConstants);

		return ProcessMSFileOrFolder(strInputFileOrFolderPath, strOutputFolderPath, true, ref eMSFileProcessingState);
	}

	public bool ProcessMSFileOrFolder(string strInputFileOrFolderPath, string strOutputFolderPath, bool blnResetErrorCode, ref eMSFileProcessingStateConstants eMSFileProcessingState)
	{
		...
		bool blnKnownMSDataType = false;

		FileSystemInfo objFileSystemInfo = null;

		DataRow objRow = null;
		long lngCachedSizeBytes = 0;
		...
		if (strOutputFolderPath == null || strOutputFolderPath.Length == 0) {
		...
		try {
			if (strInputFileOrFolderPath == null || strInputFileOrFolderPath.Length == 0) {
				...
				if (mUseCacheFiles && !mReprocessExistingFiles) {
					...
					if (strDatasetName.Length > 0 && mMSFileInfoDataCache.CachedMSInfoContainsDataset(strDatasetName, objRow)) {
				...
				blnSuccess = ProcessMSDataset(strInputFileOrFolderPath, ref mMSInfoScanner, strDatasetName, strOutputFolderPath);
				if (blnSuccess) {
	...
	public bool ProcessMSFileOrFolderWildcard(string strInputFileOrFolderPath, string strOutputFolderPath, bool blnResetErrorCode)
	{
		...
		try {
			...
			if ((strInputFileOrFolderPath != null) && (strInputFileOrFolderPath.IndexOf('*') >= 0 | strInputFileOrFolderPath.IndexOf('?') >= 0)) {
				...
				foreach (var fiFileMatch in diFolderInfo.GetFiles(strInputFileOrFolderPath)) {
					blnSuccess = ProcessMSFileOrFolder(fiFileMatch.FullName, strOutputFolderPath, blnResetErrorCode, ref eMSFileProcessingState);
				...
				foreach (var diFolderMatch in diFolderInfo.GetDirectories(strInputFileOrFolderPath)) {
					blnSuccess = ProcessMSFileOrFolder(diFolderMatch.FullName, strOutputFolderPath, blnResetErrorCode, ref eMSFileProcessingState);
				...
				blnSuccess = ProcessMSFileOrFolder(strInputFileOrFolderPath, strOutputFolderPath, blnResetErrorCode, ref eMSFileProcessingState);
			}
	...
	/// <returns></returns>
	/// <remarks></remarks>
	public bool ProcessMSFilesAndRecurseFolders(string strInputFilePathOrFolder, string strOutputFolderPath, int intRecurseFoldersMaxLevels)
	{
		...
		try {
			...
			if ((strInputFolderPath != null) && strInputFolderPath.Length > 0) {
	...
	private bool RecurseFoldersWork(string strInputFolderPath, string strFileNameMatch, string strOutputFolderPath, ref int intFileProcessCount, ref int intFileProcessFailCount, int intRecursionLevel, int intRecurseFoldersMaxLevels)
	{
		...
		DirectoryInfo diSubfolder = default(DirectoryInfo);

		string[] strFileExtensionsToParse = null;
		string[] strFolderExtensionsToParse = null;
		bool blnProcessAllFileExtensions = false;

		int intRetryCount = 0;
		int intExtensionIndex = 0;

		bool blnSuccess = false;
		bool blnFileProcessed = false;
		bool blnProcessedZippedSFolder = false;

		int intSubFoldersProcessed = 0;
		Hashtable htSubFoldersProcessed = default(Hashtable);
		...
		try {
			...
			blnProcessAllFileExtensions = ValidateExtensions(ref strFileExtensionsToParse);
			ValidateExtensions(ref strFolderExtensionsToParse);
		} catch (Exception ex) {
			...
			foreach (var fiFileMatch in diInputFolder.GetFiles(strFileNameMatch)) {
				...
				do {
					...
					try {
						...
						for (intExtensionIndex = 0; intExtensionIndex <= strFileExtensionsToParse.Length - 1; intExtensionIndex++) {
							if (blnProcessAllFileExtensions || fiFileMatch.Extension.ToUpper() == strFileExtensionsToParse[intExtensionIndex]) {
								blnFileProcessed = true;
								blnSuccess = ProcessMSFileOrFolder(fiFileMatch.FullName, strOutputFolderPath, true, ref eMSFileProcessingState);
		...
		if (!mAbortProcessing) {
			...
			try {
				...
				foreach ( diSubfolder in diInputFolder.GetDirectories(strFileNameMatch)) {
					intRetryCount = 0;
					do {
						try {
							...
							if (diSubfolder.Name == clsBrukerOneFolderInfoScanner.BRUKER_ONE_FOLDER_NAME) {
								blnSuccess = ProcessMSFileOrFolder(diSubfolder.FullName, strOutputFolderPath, true, ref eMSFileProcessingState);
								if (!blnSuccess) {
									intFileProcessFailCount += 1;
									blnSuccess = true;
								} else {
								...
								for (intExtensionIndex = 0; intExtensionIndex <= strFolderExtensionsToParse.Length - 1; intExtensionIndex++) {
									if (diSubfolder.Extension.ToUpper == strFolderExtensionsToParse(intExtensionIndex)) {
										blnSuccess = ProcessMSFileOrFolder(diSubfolder.FullName, strOutputFolderPath, true, ref eMSFileProcessingState);
										if (!blnSuccess) {
											intFileProcessFailCount += 1;
											blnSuccess = true;
				...
				if (intRecurseFoldersMaxLevels <= 0 || intRecursionLevel <= intRecurseFoldersMaxLevels) {
					...
					foreach ( diSubfolder in diInputFolder.GetDirectories()) {
						intRetryCount = 0;
						do {
							try {
	...
	public bool SaveParameterFileSettings(string strParameterFilePath)
	{
	...
	private void SetErrorCode(eMSFileScannerErrorCodes eNewErrorCode)
	{
		SetErrorCode(eNewErrorCode, false);
	}


	private void SetErrorCode(eMSFileScannerErrorCodes eNewErrorCode, bool blnLeaveExistingErrorCodeUnchanged)
	{
	...
	private bool ValidateExtensions(ref string[] strExtensions)
	{
		...
		for (intExtensionIndex = 0; intExtensionIndex <= strExtensions.Length - 1; intExtensionIndex++) {
			if (strExtensions(intExtensionIndex) == null) {
				strExtensions(intExtensionIndex) = string.Empty;
			} else {
				if (!strExtensions(intExtensionIndex).StartsWith(".")) {
					strExtensions(intExtensionIndex) = "." + strExtensions(intExtensionIndex);
				}

				if (strExtensions(intExtensionIndex) == ".*") {
	...
	DateTime static_WriteFileIntegrityDetails_dtLastWriteTime;
	private void WriteFileIntegrityDetails(ref StreamWriter srOutFile, int intFolderID, clsFileIntegrityChecker.udtFileStatsType[] udtFileStats)
	{
		...
		for (intIndex = 0; intIndex <= udtFileStats.Length - 1; intIndex++) {
			// Note: HH:mm:ss corresponds to time in 24 hour format
			srOutFile.WriteLine(
                intFolderID.ToString() + '\t' + 
                udtFileStats[intIndex].FileName + '\t' + 
                udtFileStats[intIndex].SizeBytes.ToString() + '\t' + 
                udtFileStats[intIndex].ModificationDate.ToString("yyyy-MM-dd HH:mm:ss") + '\t' + 
                udtFileStats[intIndex].FailIntegrity + '\t' + 
                udtFileStats[intIndex].FileHash + '\t' + dtTimeStamp.ToString("yyyy-MM-dd HH:mm:ss"));
		}

		if (DateTime.UtcNow.Subtract(static_WriteFileIntegrityDetails_dtLastWriteTime).TotalMinutes > 1) {
			srOutFile.Flush();
			static_WriteFileIntegrityDetails_dtLastWriteTime = DateTime.UtcNow;
		}

	}

	DateTime static_WriteFileIntegrityFailure_dtLastWriteTime;
	private void WriteFileIntegrityFailure(ref StreamWriter srOutFile, string strFilePath, string strMessage)
	{
		...
		if (DateTime.UtcNow.Subtract(static_WriteFileIntegrityFailure_dtLastWriteTime).TotalMinutes > 1) {
			srOutFile.Flush();
			static_WriteFileIntegrityFailure_dtLastWriteTime = DateTime.UtcNow;
		}

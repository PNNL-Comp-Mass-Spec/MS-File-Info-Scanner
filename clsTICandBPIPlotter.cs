...
public class clsTICandBPIPlotter
{
	...
	{
		udtOutputFileInfoType udtOutputFileInfo = default(udtOutputFileInfoType);
	...
	protected void AddSeries(PlotModel myplot, List<DataPoint> objPoints)
	{
		...
		object series = new LineSeries();

		if (objPoints.Count > 0) {
			object eSymbolType = MarkerType.None;
	...
	/// <remarks></remarks>
	private clsPlotContainer InitializePlot(clsChromatogramInfo objData, string strTitle, int intMSLevelFilter, string strXAxisLabel, string strYAxisLabel, bool blnAutoMinMaxY, bool blnYAxisExponentialNotation)
	{

		object intMinScan = int.MaxValue;
		object intMaxScan = 0;
		double dblScanTimeMax = 0;
		double dblMaxIntensity = 0;

		// Instantiate the list to track the data points
		object objPoints = new List<DataPoint>();


		foreach (void chromDataPoint_loopVariable in objData.Scans) {
			chromDataPoint = chromDataPoint_loopVariable;

			if (intMSLevelFilter == 0 || chromDataPoint.MSLevel == intMSLevelFilter || intMSLevelFilter == 2 & chromDataPoint.MSLevel >= 2) {
		...
		object myPlot = clsOxyplotUtilities.GetBasicPlotModel(strTitle, strXAxisLabel, strYAxisLabel);
		...
		object xVals = from item in objPointsitem.X;
		clsOxyplotUtilities.UpdateAxisFormatCodeIfSmallValues(myPlot.Axes(0), xVals, true);

		object yVals = from item in objPointsitem.Y;
	...
	public bool SaveTICAndBPIPlotFiles(string strDatasetName, string strOutputFolderPath, ref string strErrorMessage)
	{
	...
	protected void RemoveZeroesAtFrontAndBack(ref clsChromatogramInfo objChrom)
	{
		...
		int intIndex = 0;
		int intZeroPointCount = 0;
		int intPointsChecked = 0;
		int intIndexNonZeroValue = 0;
	...
	protected class clsChromatogramInfo
	{
		...
		{
			if ((from item in mScanswhere item.ScanNum == intScanNumberitem).Any()) {
				throw new Exception("Scan " + intScanNumber + " has already been added to the TIC or BPI; programming error");
			}

			object chromDataPoint = new clsChromatogramDataPoint {

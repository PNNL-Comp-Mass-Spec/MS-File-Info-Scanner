...
Use /I to specify the name of a file or folder to scan; the path can contain the
wildcard character *
...
number after /S (like /S:2) to limit the level of subfolders to examine. Use /IE
to ignore errors when recursing.
...
TMT labelled and it is important to detect the reporter ions in the MS/MS spectra
...
is only used if you specify an Input Folder and use a wildcard; you will

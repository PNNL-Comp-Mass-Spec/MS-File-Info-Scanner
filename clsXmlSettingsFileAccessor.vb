...
Public Class XmlSettingsFileAccessor
    ...
    Public Function SectionPresent(ByVal sectionName As String) As Boolean
		Dim strSections As Collections.Specialized.StringCollection
	...
	Private Function CacheKeyNames(ByVal sectionName As String) As Boolean
		...
		Dim strKeys As Collections.Specialized.StringCollection
		Dim intIndex As Integer
	...
	Protected Class XMLFileReader
		...
		Private sections As Collections.Specialized.StringCollection = New Collections.Specialized.StringCollection
		...
		''' </summary>
		Private Function GetRoot() As Xml.XmlElement
		...
		''' </summary>
		''' <return>The function returns the last section as Xml.XmlElement.</return>
		Private Function GetLastSection() As Xml.XmlElement
		...
		''' <summary>
		''' The function gets a section as Xml.XmlElement.
		''' </summary>
		''' <param name="sectionName">The name of a section.</param>
		''' <return>The function returns a section as Xml.XmlElement.</return>
		Private Function GetSection(ByVal sectionName As String) As Xml.XmlElement
			If (Not (sectionName = Nothing)) AndAlso (sectionName <> "") Then
				sectionName = SetNameCase(sectionName)
				Return CType(m_XmlDoc.SelectSingleNode("//section[@name='" & sectionName & "']"), Xml.XmlElement)
			End If
			Return Nothing
		End Function
		...
		Private Function GetItem(ByVal sectionName As String, ByVal keyName As String) As Xml.XmlElement
			Dim section As Xml.XmlElement
			If (Not keyName Is Nothing) AndAlso (keyName <> "") Then
				keyName = SetNameCase(keyName)
				section = GetSection(sectionName)
				If (Not section Is Nothing) Then
					Return CType(section.SelectSingleNode("item[@key='" + keyName + "']"), Xml.XmlElement)
				End If
			End If
			Return Nothing
		End Function
		...
		Private Function DeleteSection(ByVal sectionName As String) As Boolean
			Dim section As Xml.XmlElement = GetSection(sectionName)
			If Not section Is Nothing Then
		...
		Private Function DeleteItem(ByVal sectionName As String, ByVal keyName As String) As Boolean
			Dim item As Xml.XmlElement = GetItem(sectionName, keyName)
			If Not item Is Nothing Then
		...
		Public Function GetXMLValue(ByVal sectionName As String, ByVal keyName As String) As String
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
			Dim N As Xml.XmlNode = GetItem(sectionName, keyName)
		...
		''' </summary>
		Public Function GetIniComments(ByVal sectionName As String) As Collections.Specialized.StringCollection
		...
		Public Function GetXmlSectionComments(ByVal sectionName As String) As Collections.Specialized.StringCollection
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
			Dim sc As Collections.Specialized.StringCollection = New Collections.Specialized.StringCollection
			Dim target As Xml.XmlNode
		...
		''' </summary>
		Public Function SetIniComments(ByVal sectionName As String, ByVal comments As Collections.Specialized.StringCollection) As Boolean
		...
		'''<return>The function returns a Boolean that shows if the change was done.</return>
		Public Function SetXMLComments(ByVal sectionName As String, ByVal comments As Collections.Specialized.StringCollection) As Boolean
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
			Dim target As Xml.XmlNode
			Dim nodes As Xml.XmlNodeList
			Dim N As Xml.XmlNode
			Dim s As String
			Dim NLastComment As Xml.XmlElement
			If sectionName Is Nothing Then
				target = m_XmlDoc.DocumentElement
			Else
				target = GetSection(sectionName)
			...
			If Not target Is Nothing Then
				...
				For Each s In comments
					...
					N.InnerText = s
					NLastComment = CType(target.SelectSingleNode("comment[last()]"), Xml.XmlElement)
		...
		Private Sub UpdateSections()
			sections = New Collections.Specialized.StringCollection
		...
		Public ReadOnly Property AllSections() As Collections.Specialized.StringCollection
			Get
				If Not Initialized Then
		...
		Private Function GetItemsInSection(ByVal sectionName As String, ByVal itemType As XMLItemTypeEnum) As Collections.Specialized.StringCollection
			Dim nodes As Xml.XmlNodeList
			Dim items As Collections.Specialized.StringCollection = New Collections.Specialized.StringCollection
			Dim section As Xml.XmlNode = GetSection(sectionName)
			Dim N As Xml.XmlNode
			If section Is Nothing Then
				Return Nothing
			Else
		...
		Public Function AllKeysInSection(ByVal sectionName As String) As Collections.Specialized.StringCollection
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
		...
		Public Function AllValuesInSection(ByVal sectionName As String) As Collections.Specialized.StringCollection
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
		...
		Public Function AllItemsInSection(ByVal sectionName As String) As Collections.Specialized.StringCollection
			If Not Initialized Then Throw New XMLFileReaderNotInitializedException
		...
		''' <param name="doc">The name of the Xml.XmlDocument.</param>
		''' <returns>True if success, false if not a recognized line format</returns>
		''' <remarks>Returns True for blank lines</remarks>
		Private Function ParseLineManual(ByVal strLine As String, ByRef doc As Xml.XmlDocument) As Boolean
		...
		''' <summary>It gets the Xml.XmlDocument.</summary>
		...
		Public ReadOnly Property XML() As String
			Get
				...
				Using sw As StringWriter = New StringWriter(sb)
					Using xw As Xml.XmlTextWriter = New Xml.XmlTextWriter(sw)
						xw.Indentation = 3
